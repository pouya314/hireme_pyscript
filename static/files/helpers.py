...
def not_answered(question):
    return question_encountered_first_time(question) or question_has_validation_errors(question)


def question_encountered_first_time(question):
    return (question["provided_answer"] is None and 
            question["validation_errors"] == [] and 
            question["acceptance_errors"] == [])


def question_has_validation_errors(question):
    return (question["provided_answer"] is not None and 
            question["validation_errors"] != [] and 
            question["acceptance_errors"] == [])
