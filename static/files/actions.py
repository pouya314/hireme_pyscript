...
def initialize_action(state, action):
    ...
    state_with_eligibility_qs = assoc_in(
        state, ["eligibility_questions"], eligibility_questions
    )
    return assoc_in(
        state_with_eligibility_qs, ["application_questions"], application_questions
    )
